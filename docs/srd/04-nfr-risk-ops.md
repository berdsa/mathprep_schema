...
NFR thresholds unchanged, see `02-requirements.md`. Degradation behavior, updated for the split: if `taskgen` is down, `grader` is unaffected (no runtime dependency between them) — students can still submit and get graded on already-issued items, they just can't receive a new set until `taskgen` recovers. If `grader` is down, already-generated items simply can't be answered yet; nothing is lost, since submissions are client-initiated and the QR page can retry.
...
|---|---|
| Bot ↔ taskgen | Webhook signature/secret verification (spoofing); TLS + idempotency key (tampering); per-update-id logging (repudiation); per-chat rate limit (DoS) |
| QR Web ↔ grader | Signed, time-boxed, single-item-scoped token, no raw bearer stored; server re-derives item context from the token; non-sequential UUIDv4 item_id `[ASSUMPTION]`; per-token attempt cap; `raw_input` max-length enforced pre-parse |
| taskgen ↔ PostgreSQL | Parametrized queries only; statement timeout; DB role scoped to only the tables it needs (`GENERATION_REQUEST`, `TASK_SET`, `TASK_INSTANCE`, `TASK_TYPE` read, `EVENT_LOG` insert-only, `MASTERY_TOPIC` read) |
| grader ↔ PostgreSQL | Parametrized queries only; statement timeout; DB role scoped separately (`SUBMISSION` write, `TASK_INSTANCE` read-only — **grader must never be able to write `TASK_INSTANCE`**, that would let a compromised grader forge what a student was shown; `EVENT_LOG` insert-only; `MASTERY_TOPIC` read+write) |
| taskgen ↔ grader | **No boundary exists** — they never talk to each other directly, by design (ADR-006). The only shared surface is the database, already covered above |
| analytics ↔ PostgreSQL *(later phase)* | Read-only role on `EVENT_LOG` and derived tables only — never write access to any operational table |

**New row this pass, driven by the service split:** because `taskgen` and `grader` are separate deployables with separate DB roles, a compromise of one no longer automatically implies write access to the other's tables — this is a genuine security improvement of ADR-006 over the old single-role monolith, worth naming as a benefit, not just a cost.
...
## Failure modes
Unchanged (`generator panics mid-batch`, `DB unavailable during submission`), with ownership reattributed: the first to `taskgen`, the second to `grader`.
