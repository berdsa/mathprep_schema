...
For a senior Go developer implementing `schema`, `taskgen`, and `grader` as three independent git repositories (ADR-006), integrated only through a shared PostgreSQL schema (ADR-006, ADR-007). Ordering follows the operator's explicit instruction: **DB and dictionaries first → one task type end to end → multi-stage confirmation → repeat one type at a time, grade 1 through university → journal from day one, analytics as its own later phase.**
...
## Phase 0 — Schema, tables, dictionaries (`schema` repo)

**Scope:**
1. Repository skeleton: `schema`, Go module, migration tool of the developer's choice (e.g. `golang-migrate` or `goose` — either is a reasonable, standard fit for this stack; not mandated further, developer's judgment) `[ASSUMPTION]`.
...
- `schema` tagged `v0.1.0`, published so the next two repos can `go get` it.
...
**CAS infrastructure gate — satisfied:** the independent `cas` service and shared security boundary (`OPEN-06`, `backend-integration.md` §3) are implemented and signed off by Ken; record: `taskgen/docs/signoffs/OPEN-06.md`. The boundary includes whitelist-grammar tokenization, no `eval()`, AST node/depth caps, separate process/resource limits and hard kill, per-session rate limit, and DoS fixtures. Before any Grade 10–11 CAS-adjacent or University type is used, that type must still pass its own specification, tests, and Confirmation Gate. This remains a per-type requirement, not an open shared-infrastructure gate.
...
**Exit criteria for Phase 2+ as a whole:** every task type in `math-task-catalog.md` (excluding the explicitly out-of-scope categories: graph plotting, geometric construction, open-ended proofs — `00-scope-lock.md` §3) has reached at least `GATED` status, or has an explicitly logged reason it hasn't (e.g., awaiting its per-type Confirmation Gate or blocked on a genuinely unresolvable multiple-valid-answer form per the cartridge's own exclusion rule).

---

## Deferred phase — Analytics (`analytics` repo, started only after the above is substantially stable)
...
3. A new `schema` migration for whatever rollup table this needs — owned by `schema`, not by `analytics` itself (§1 of `backend-integration.md`).
...
**Explicitly NOT part of this deferred phase, restated because it's the easiest thing to misread:** `MASTERY_TOPIC`'s incremental EMA update is core functionality, already built in Phase 1/2 inside `grader`. It does not wait for this phase.

---

## Cross-cutting notes for the developer, gathered from every chapter's fine print

- Never hardcode a dictionary value (verdict, reason code, domain, etc.) as a raw string literal in `taskgen` or `grader` application code — import it from `schema`. A string literal duplicating a dictionary value is a code-review rejection under Stage B above.
