...
**[BLOCKING] CAS has unbacked local commits.** `cas` currently reports `main...origin/main [ahead 2]`; `git ls-remote origin HEAD` failed with `Permission denied (publickey)`. The current CAS HEAD and two commits ahead of the cached remote-tracking ref are not confirmed on GitLab; those commits are a single-point-of-failure on Ken’s local machine until SSH is restored and the branch is pushed. Evidence: `git -C cas status -sb`, `git -C cas rev-list --count origin/main..HEAD` → 2, failed `git ls-remote origin HEAD`; origin is `git@gitlab.com:math_gen/cas.git`.
